...
def display_transform_status_message(enabled: bool, *, imagecms_module: Any) -> str:
    ...
    if display_profile is None:
        return '显示变换：未找到显示器配置文件，使用原始预览'
    return f'显示变换：已应用显示器配置文件（{profile_name}）'
...
def apply_display_transform(
    ...
    *,
    ...
    display_profile, profile_name = display_profile_details(imagecms_module=imagecms_module)
    if display_profile is None:
        return np.uint8(np.clip(image_data, 0.0, 1.0) * 255), '显示变换：未找到显示器配置文件，使用原始预览'
    ...
    return np.asarray(transformed_image, dtype=np.uint8), f'显示变换：已激活（{profile_name}）'
...
def prepare_output_display_image(
    ...
    *,
    ...
    try:
        ...
        return preview_image, '显示变换：变换失败，使用原始预览'
