...
DEFAULT_CONTROLS_PANEL_WIDTH = 480
...
def _build_viewer_panel(
    ...
    *,
    ...
    rotate_ccw_button = QPushButton('↺ 逆时针')
    ...
    rotate_cw_button = QPushButton('↻ 顺时针')
    ...
    zoom_100_button.setToolTip('屏幕像素与图像像素 1:1 对应')
    ...
    zoom_200_button.setToolTip('2 个屏幕像素对应 1 个图像像素')
    ...
    zoom_400_button.setToolTip('4 个屏幕像素对应 1 个图像像素')
    if on_zoom_400 is not None:
        zoom_400_button.clicked.connect(on_zoom_400)

    home_button = QPushButton('⌂ 复位')
...
def build_controls_panel(
    ...
    *,
    ...
    on_toggle_split: Callable[[], None] | None = None,
) -> QWidget:
    from spektrafilm_gui.help_dialog import show_help
    ...
    help_btn.setToolTip('参数使用指南')
    help_btn.setStyleSheet(btn_style)
    help_btn.clicked.connect(lambda: show_help(panel))

    reset_btn = QPushButton('↺')
    reset_btn.setToolTip('恢复出厂默认参数')
    reset_btn.setStyleSheet(btn_style)
    if on_reset is not None:
        reset_btn.clicked.connect(on_reset)

    split_btn = QPushButton('⇆')
    split_btn.setToolTip('原图 / 胶片效果全屏切换，快捷键 Y')
    split_btn.setStyleSheet(btn_style)
    if on_toggle_split is not None:
        split_btn.clicked.connect(on_toggle_split)

    corner_layout.addWidget(split_btn)
    corner_layout.addWidget(reset_btn)
    corner_layout.addWidget(help_btn)
    panel.setCornerWidget(corner, QtCore.Qt.TopRightCorner)
    # ── 图层列表内容（主面板底部用）──
    ...
    panel.addTab(
        _wrap_scrollable(
            _build_controls_tab(
                ...
                widgets.display,
                CollapsibleSection('图层列表', napari_layers_content, expanded=False),
            ),
        ),
        '主面板',
    )
    ...
    panel.addTab(
        _wrap_scrollable(_build_controls_tab(
            widgets.halation,
            widgets.grain,
            widgets.grain_crop_preview,
            widgets.glare,
            widgets.camera_diffusion,
            widgets.diffusion,
            widgets.preflashing,
            widgets.couplers,
        )),
...
def build_main_window(
    ...
    *,
    ...
    main_window.setWindowTitle('Spektrafilm — 胶片光谱模拟  ·  Andrea Volpato')
    main_window.setWindowIcon(QIcon())
    main_window.resize(DEFAULT_CONTROLS_PANEL_WIDTH + DEFAULT_VIEWER_SPLITTER_WIDTH, 1000)
