...
def _apply_special(params: RuntimePhotoParams, state: GuiState) -> None:
    ...
    params.film_render.density_curve_gamma = max(float(state.special.film_gamma_factor), 0.01)
    params.print_render.density_curve_gamma = max(float(state.special.print_gamma_factor), 0.01)
...
def _apply_grain(params: RuntimePhotoParams, state: GuiState) -> None:
    # 颗粒大小 ≤ 0 时物理上无意义（grain.py 里会除以零），等同于关闭颗粒
    if state.grain.particle_area_um2 <= 0:
...
def _apply_scanner(params: RuntimePhotoParams, state: GuiState) -> None:
    ...
    # white_level 必须严格大于 black_level，否则 color_reference.py 里 m=0 除以零崩溃
    black = float(state.simulation.scan_black_level)
    white = float(state.simulation.scan_white_level)
    if white <= black:
        white = black + 0.01
    params.scanner.black_level = black
    params.scanner.white_level = white
